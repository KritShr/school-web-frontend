    ...
    <div className="flex px-5 py-2 justify-between items-center">
      <div className="flex items-center gap-3 cursor-pointer" onClick={() => moveTo('/')}>
        <img
          ...
          loading="lazy"
          alt=""
          src="/image-3@2x.png"x
        />
        <h2 className="font-bold text-4xl -text--text-default-default">Creative Learners' Academy</h2>
      </div>

      <div className="flex items-center gap-7 -text--text-default-default">
        <h3 className={`font-semibold text-2xl cursor-pointer hover:-text--medium ${currentPath === '/' ? '-text--medium' : ''}`} onClick={() => moveTo('/')}>Home</h3>
        <h3 className={`font-semibold text-2xl cursor-pointer hover:-text--medium ${currentPath === '/about-us' ? '-text--medium' : ''}`} onClick={() => moveTo('/about-us')}>About Us</h3>
        <h3 className={`font-semibold text-2xl cursor-pointer hover:-text--medium ${currentPath === '/management' ? '-text--medium' : ''}`} onClick={() => moveTo('/management')}>Management</h3>
        <h3 className={`font-semibold text-2xl cursor-pointer hover:-text--medium ${currentPath === '/facilities' ? '-text--medium' : ''}`} onClick={() => moveTo('/facilities')}>Facility</h3>
